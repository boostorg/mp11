...
template <class T, class... Rest> struct mp_find_impl_2<T, Rest...>: mp_find_impl_2<Rest...>
{
    ...
    using parent_tag = typename mp_find_impl_2<Rest...>::tag;
...
{
    using mp_find_if_impl_2<Rest...>::get;
    using parent_tag = typename mp_find_if_impl_2<Rest...>::tag;
    struct tag : parent_tag {};
...
template <template <class...> class L, class... T, template<class...> class P> struct mp_find_if_impl<L<T...>, P>
{
    using helper = mp_find_if_impl_2<T...>;
    using tag = typename helper::tag;
    using R = decltype(helper::get<P>(tag()));
    using type = mp_size_t<sizeof...(T) - R::value>;
};
